...
RowLayout {
    ...
    Resource {
        ...
        value1: Math.min(1, SystemUsage.fanSpeed / 3000)  // Normalize to max 3000 RPM
        value2: SystemUsage.batteryPercentage / 100

        label1: `${Math.round(SystemUsage.fanSpeed)}`
        label2: `${SystemUsage.powerDraw.toFixed(1)}W`

        sublabel1: qsTr("Fan RPM")
        sublabel2: qsTr("Power Draw")

        fg1: Colours.palette.m3tertiary
        fg2: SystemUsage.powerDraw > 15 ? Colours.palette.m3error : Colours.palette.m3primary
        bg1: Colours.palette.m3tertiaryContainer  
        bg2: Colours.palette.m3primaryContainer
    }
